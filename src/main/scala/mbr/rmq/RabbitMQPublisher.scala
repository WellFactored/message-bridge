...
class ExchangePublisher[F[_]: Defer: Sync](channel: Channel, exchangeName: ExchangeName) extends RabbitMQPublisher[F] with EffectfulLogging[F] {
  override def publish(routingKey: String, properties: BasicProperties, body: String): F[Unit] = {
    delay(channel.basicPublish(exchangeName.value, routingKey, properties, body.getBytes))
  }
